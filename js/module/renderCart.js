...
import { getElement , changeFirstUpperCase} from "./util.js";
...
const renderCartList=async ()=>{
  ...
  if(cartControl.cartData.length){
    ...
    //запрос на сервер
    const cardsDataPromises=cartControl.cartData.map(async (item)=>{
      return await getData(`https://amused-ripple-clove.glitch.me/api/products/${item.id}`);
    });
    //вытаскиваем массив из промиса
    ...
    //создадим li елемент
    const cardCart=getElement('li','modal-cart__item');
    cardCart.innerHTML=`
    <picture>
...
const submitOrder= async (evt)=>{
    ...
    try{
     const responce=await fetch('https://amused-ripple-clove.glitch.me/api/orders/',{
       method:'POST',
       headers:{
     ...
     if(!responce.ok){
     throw new Error('Failed to sumbit order');
     }
     ...
     `;
     [nextBtn,prevBtn,submitBtn].forEach(item => item.disabled=true);
     } catch (err) {
...
export const modalCartController=()=>{
  renderCartList();
  //вешаем обработчик удаления по клику из корзины на родителя
